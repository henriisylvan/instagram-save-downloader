...
const changeCredentials = async (options = { force: false }) => {
    const preRequisitsToAsk = options.force || !credentials.obj.username || !credentials.obj.password;

    const changeCredentialsBool = preRequisitsToAsk || await interface.askChangeCredentials(credentials.obj.username); 

    if (changeCredentialsBool) {
        credentials.obj = await interface.getCredentialsInput(preRequisitsToAsk);
