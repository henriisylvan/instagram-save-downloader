...
const booleanReadline = async (promptLine, choices = { true: ['y'], false: ['n'] }, options = {}) => {
    ...
    const formattedResponse = options.sensitiveCase ? response : response.toLowerCase();

    if (choices.true.includes(formattedResponse) || choices.false.includes(formattedResponse))
        return choices.true.includes(formattedResponse);
