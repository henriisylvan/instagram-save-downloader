...
const retriveAccountPostsUrls = async log => {
    try {
        ...
        const amountRetrivedUrls = log.obj.posts.reduce((amount, post) => {
            const findedPost = data.find(retrivedPost => retrivedPost.url == post.url);
...
const selectAccountPostsLogForDownload = async () => {
    ...
    if (!safeUrls) {
        const retrived = await retriveAccountPostsUrls(selectedLog);
