...
const generateCollectionsStructure = () => {
    ...
    return additionalCollections.concat(collectionsStructure);
};
...
const separateItemsByCollection = (items, collectionsStructure) => {
    ...
    for (const collectionIndex in collectionsStructure) {
        ...
        switch (collection) {
            ...
            default:
                const collectionPosts = allPost.filter(post => post.collectionIds.includes(code));
            
                collectionsStructure[collectionIndex].posts.push(...collectionPosts);
        };
    };
