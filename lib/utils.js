...
const getOrdinal = number => {
    ...
    return number + (sufix[(value - 20) % 10] || sufix[value] || sufix[0]);
};
...
const convertSecondsToTimeFormat = seconds => {
    const indexs = range(3).reverse();
    
    const timeArr = indexs.map(index => Math.floor(seconds / 60 ** index) % (index == 2 ? Infinity : 60));
    const timeStrArr = timeArr.map(num => num.toString().padStart(2, 0));

    return timeStrArr.join(':');
};

const padBoth = (str, maxLength, prefix, sufix = prefix) => {
    if (str.length >= maxLength) return str;

    const lengthSplited = (maxLength - str.length) / 2;

    const prefixStr = prefix.repeat(Math.floor(lengthSplited));
    const sufixStr = sufix.repeat(Math.ceil(lengthSplited));

    return prefixStr + str + sufixStr;
};
